...
class CocoDetection_share_memory(torchvision.datasets.VisionDataset):
    def __init__(
        self, img_folder, ann_file, transforms, return_masks=False, remap_mscoco_category=False, share_memory=True
    ):
    ...
    def __getitem__(self, idx):
        ...
        image, target = (
            self.prepare(image, target, category2label=mscoco_category2label)
            if self.remap_mscoco_category
            else self.prepare(image, target)
        )
