...
from src.data.coco import CocoDetection_share_memory
...
def main(**kwargs):
    ...
    dataloader = DataLoader(dataset=dataset, worker_init_fn=hook_pid, batch_size=32, num_workers=2, shuffle=False)
...
def main2(**kwargs):
    ...
    ctx = torch.multiprocessing.start_processes(
        worker, (ds,), nprocs=4, join=False, daemon=True, start_method=start_method
    )
