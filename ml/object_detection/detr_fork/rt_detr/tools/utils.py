...
def fit(
    ...
    for epoch in range(last_epoch + 1, epoch):
        ...
        train_dataloader.sampler.set_epoch(
            epoch
        ) if dist_utils.is_dist_available_and_initialized() else train_dataloader.set_epoch(epoch)
...
def val(model, weight_path, val_dataloader, criterion, use_amp=True, use_ema=True):
    if weight_path != None:
        state = (
            torch.hub.load_state_dict_from_url(weight_path, map_location="cpu")
    ...
    postprocessor = RTDETRPostProcessor(
        num_top_queries=300, remap_mscoco_category=val_dataloader.dataset.remap_mscoco_category
    )
