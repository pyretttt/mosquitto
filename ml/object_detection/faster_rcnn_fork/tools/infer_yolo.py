...
def load_model_and_dataset(args, custom_batch_size: int = 0):
    ...
    yolo.eval()
    yolo.to(device)
    yolo.load_state_dict(torch.load(os.path.join(train_config['task_name'],
                                                              train_config['ckpt_name']),
                                                 map_location=device))
    return yolo, voc, test_dataset
...
def infer(args):
    ...
    # yolo.roi_head.low_score_threshold = 0.7
    
    for sample_count in tqdm(range(10)):
        random_idx = random.randint(0, len(voc))
        im, _, fname, yolo_target = voc[random_idx]
