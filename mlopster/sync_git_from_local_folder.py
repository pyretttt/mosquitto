...
def copy_tree_merge(src: Path, dst: Path) -> None:
    ...
    shutil.copytree(src, dst, dirs_exist_ok=True, ignore=shutil.ignore_patterns('no_index', '.git'))
...
def main() -> None:
    ...
    parser.add_argument(
        "--dest",
        type=Path,
        default=root / "no_index" / "mlopster",
        help="Path to the mirror git working tree ",
    )
    parser.add_argument(
        "--source",
        type=Path,
