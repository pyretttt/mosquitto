...
use crate::features::command::{Command, CommandPallette};
...
use crate::features::window_size::{WindowSize, WindowSizeAction, window_size_reducer};
    ...
    match action {
        ...
        Action::WindowSize(action) => {
            if let Some(Overlay::WindowSize(ref mut window_size)) = app_state.overlay {
                ...
                assert!(
                    false,
                    "Action dispatched with no window size overlay {:?}",
                    action
                );
            }
        }
        ...
        Event::Crossterm(crossterm_event) => {
            match crossterm_event {
                        ...
                        match key_event.code {
                            ...
                            KeyCode::Enter => {
                                ...
                                env.fire_and_forget(async move {
                                    ...
                                    _ = sender.send(
                                        Action::LoadingPage(LoadingPageAction::Finished).into(),
                                    );
                ...
                crossterm::event::Event::Resize(width, height) => {
                    ...
                    let invalid_size = new_size.width < env.ui.required_window_size.width
                        || new_size.height < env.ui.required_window_size.height;
                    match &mut app_state.overlay {
